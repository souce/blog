...
```
范例代码中存在好些个异常处理分支，一但出现异常则开始回收变量准备退出，相信大家在工作中没少见到这样的代码，也没少在这样的代码上遇到“double free”或“内存溢出”，它存在有几个问题：
...
1. 重复的错误处理流程占据了大部分篇幅，代码可读性差。
	...
	大多数人在写这种重复内容的代码（错误处理逻辑）都用的复制粘贴的方法，这很容易造成遗漏，范例中的三个错误处理都有细微的不同，混合着10多个变量的free动作在里面时，不细看确实很难发现。
...
2.基于第一条规则（很重要），制定出“退出时清理逻辑”，遇到需要结束函数的异常时goto到该逻辑执行退出清理动作。

“退出时清理逻辑”可以有多个，根据不同情况可以采用不同的退出方式。

统一“退出时清理逻辑”的好处是实现了代码复用，相比复制粘贴，它不那么容易遗漏，而且代码“流畅性”大大增加，在看代码时不会总是被“异常处理分支”打断，代码更加精简，流程更加清晰。
...
“统一的清理逻辑，使用goto跳转” 这样会扩大变量的生命周期，使得变量的生命周期大于其实际的使用范围，在资源受限或性能要求高的程序中，如果变量对应的值在堆中占用空间很多，它的生命周期又太长以至于不能尽快的free，对程序的性能会有一定的损耗（可以通过拆分函数来缩小变量的生命周期，虽然多了函数调用开销，但大部分情况下可以缓解这个情况）...

在工作中项目组里的同事技术水平必然参差不齐，对代码整洁度的追求也不尽相同，如果不制定规范，那么将来维护、升级这个项目将会是一个巨大的挑战，所以我认为牺牲部分“效率”（开发效率、运行效率）和“代码灵活性”来换取“可读性”和“可维护性”是笔不错的买卖。
