...
* “统一的清理逻辑，使用goto跳转” 这样会扩大变量的生命周期，使得变量的生命周期大于其实际的使用范围，在资源受限或性能要求高的程序中，如果变量对应的值在堆中占用空间很多，且生命周期又太长以至于不能尽快的free，对程序的性能会有一定的损耗（可以通过拆分函数来缩小变量的生命周期，虽然多了函数调用开销，但在一定程度上可以缓解上述情况）...
