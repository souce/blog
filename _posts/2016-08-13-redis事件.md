...
下文中出现的“事件库们”，指的是redis实现的 ae_epoll ae_evport ae_kqueue ae_select。
...
void aeMain(aeEventLoop *eventLoop) {
    ...
    while (!eventLoop->stop) { //停止标志，一旦接受了退出信号则会修改此，退出主循环，结束进程
        ...
        aeProcessEvents(eventLoop, AE_ALL_EVENTS); //处理文件事件，和时间时间
    }
}
```
...
aeFileEvent是"文件事件结构体"；aeFiredEvent是"已触发事件结构体"；
暂时不详解它们之间的区别，先继续往下看。
