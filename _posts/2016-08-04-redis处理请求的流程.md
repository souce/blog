...
以上就是redis接收客户端的请求，处理，响应的全部过程，在这个过程中有几个回调：
...
1. 在接受客户端请求的时候需要重试，redis的事件库有多种实现方式，在编译期间通过宏定义会确定采用何种实现方式，所以redis对于事件采用的是一种比较慎重的态度，例如在acceptTcpHandler的时候它有个max--的循环，这个循环意义重大：
	
	1. 如果还有客户端连接，那么不用等到下一次的事件循环了，现在就处理掉它
	1. 防止事件使用边缘模式产生的遗漏问题:若当前正在处理“读”事件，那么此时发来的请求将不会再触发事件

1. 在读数据的时候，要判断好偏移量，因为网络问题，一次读的事件回调是有可能读不完的，那么后续还会继续回调这个读取逻辑。
