...
以上就是redis服务端接收客户端的请求，处理、响应的全部过程，在这个过程中有几个回调我们要记住：
...
1. 在使用事件库开发server端时要注意“边缘触发”问题，redis的事件库有多种实现方式，在编译期间通过宏定义会确定采用何种实现方式，所以redis对于事件采用的是一种比较慎重的态度，例如在acceptTcpHandler的时候它有个max--的循环，这个循环意义重大：
